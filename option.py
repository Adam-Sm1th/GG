...
def parse_args():
    ...
    parser.add_argument("--model_key", type=str, default="stabilityai/stable-diffusion-3.5-medium") # stabilityai/stable-diffusion-3.5-medium
    parser.add_argument("--guidance_scale", type=float, default=4.0) # 4.0 for SDv35M diversity and image quality
    parser.add_argument("--num_inference_steps", type=int, default=40) # 40 for SDv35M

    # inversion settings
    parser.add_argument("--num_inversion_steps", type=int, default=50)

    # watermark settings
    parser.add_argument("--key_channel_idx", type=list, default=[0, 1, 2, 3]) # the channel to embed the session key. For SDv35M, it is [0, 1, 2, 3].
