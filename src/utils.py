...
def image_distortion(img, seed, args):
    ...
    if args.jpeg_ratio is not None:
        buffer = BytesIO()
        img.convert("RGB").save(buffer, format="JPEG", quality=args.jpeg_ratio)
        buffer.seek(0)
    ...
    if args.gaussian_std is not None:
        ...
        noised = np.array(img).astype(np.float32) + g_noise
        img = Image.fromarray(np.clip(noised, 0, 255).astype(np.uint8))
